...
const roomIcons: Record<string, React.ReactNode> = {
  ...
  Bath: <Bath className="w-5 h-5 text-emerald-600" />,
  Bed: <Bed className="w-5 h-5 text-emerald-600" />,
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-serif font-bold text-slate-900 tracking-tight">
            Detailed Clean Checklist Matrix
          </h2>
          ...
          {CLEANING_CHECKLIST_DATA.map((section, idx) => (
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-xs sm:text-sm font-bold transition-all flex items-center gap-2.5 ${
                activeRoomIndex === idx
                  ? "bg-emerald-800 text-white shadow-md shadow-emerald-950/20 ring-2 ring-emerald-800"
                  : "bg-slate-100 text-slate-700 hover:bg-slate-200"
              }`}
            >
              {roomIcons[section.icon] || <Sparkles className="w-4 h-4 text-emerald-600" />}
        ...
        <div className="max-w-4xl mx-auto rounded-3xl border border-slate-200 bg-white shadow-xl overflow-hidden">
          ...
          <div className="grid grid-cols-12 bg-slate-900 text-white p-4 sm:p-5 text-xs sm:text-sm font-bold border-b border-slate-800 items-center">
            ...
            <div className="col-span-2 text-center">
              <span className="text-emerald-400 block text-[11px] uppercase">Move-Out</span>
            </div>
          </div>
          ...
          {/* Bottom Pro Tip Box */}
          <div className="p-5 bg-emerald-50/80 border-t border-emerald-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <a
              href="#quote-engine"
              className="px-5 py-2.5 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white font-bold text-xs shrink-0 flex items-center gap-1.5 shadow-sm"
            >
